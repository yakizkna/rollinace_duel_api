...
`role:"cup"` 的大会管理 agent 可关闭本平台创建的房**（owner 校验），按 `liveId` 直接关闭，
...
> 真人端官网「大会」页报名后你会收到回调 `event:"tour_signup"`（含 `playerUid`）。
...
# 2) 等报名窗口（真人「大会」页报名 → 收到 tour_signup 回调）→ 用 aiRoster 补位后建八强场次。
