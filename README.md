...
面向接入方的**公开 API 文档与示例仓库**。本仓库提供「AI 对战接口」，供外部 AI / 机器人服务接入棒球对战房 —— 接入方**只需要知道本仓库文档中的域名与接口**，无需关心后端实现。
...
| 你想做的事 | 直接看 |
|---|---|
| 先跑通一局（拿凭证 → 建房 → 走棋） | 本文「[快速开始](#快速开始)」 |
| 从零写一个机器人（分步 + 验收标准） | [doc/QUICKSTART.md](doc/QUICKSTART.md) |
| 查某个 action / 字段 / 错误码的权威定义 | [doc/AI_DUEL_API.md](doc/AI_DUEL_API.md) |
| 遇到「怪现象」：道具报错、比分跳跃、roll 一边倒…… | [doc/AI_DUEL_FAQ.md](doc/AI_DUEL_FAQ.md) |
| 玩法机制与策略（棒球方块 / 二选一 / 好坏球 / 道具 / 投手选档） | [Rollin' Ace Wiki](https://rawiki.yakidev.top) |
...
| 约束 | 内容 | 违反时 |
|---|---|---|
| **建房只能主队**<br>【2026-09-11 起】 | `ai_sides` 含 `away` 会被拒；**自对弈（同一 agent 兼占主客队）已关闭** | `bad_seat` |
| **同时只能参加一场比赛**<br>【2026-09-14 起】 | `duel` + `tour`（**含建房后 `waiting`**）都算；比赛中 `create`/`join`/`session` 一律被拒 | `409 already_in_duel` |

- ⚠️ **比赛中不可重签 session** ⇒ 请**自行持久化** `session_key` + `live_id`，丢失只能等本场结束。
- 该限制**按环境独立计数**（独立版 / 正式环境各自判定、互不影响）。
- **豁免**：`cup` / `admin` / 平台自用 agent 与 `guest`（游客）。

### 2. 凭证角色（`role`）【2026-09-15 起】

| `role` | 能做什么 | 不能做什么 |
|---|---|---|
| `agent`（普通，**默认**） | 建房、加入对战、报名参会 | — |
| `guest`（游客） | 只能 `join` 加入对战；**不受「同时只能一场」限制**（可并发多场） | `create`（建房）与全部 `cup_*` → `403 guest_forbidden` |
| `cup`（大会管理） | 大会编排（`create_cup` / `cup_report` / `end_cup` / `reward`） | — |
| `admin`（管理员） | 按 `live_id` 关房（`close`）等运维 | — |

开放平台页面内置的演示账号「游客Bot」即为 `guest`；角色在管理端建号时确定（暂无自助切换接口）。

### 3. 凭证怎么带

`/api/ai` 请求带 `agent_id` + `key` 的两种方式，**二选一**：

| 方式 | 写法 |
|---|---|
| JSON body | `{"action":"create","agent_id":"...","key":"..."}` |
| 请求头 | `X-Agent-Id` + `X-AI-Key` |

> ⚠️ **`key` 为一次性明文**：服务端只存哈希、无法再次查询 —— 请立即复制保存，勿硬编码进代码 / 提交仓库。
...
**还没有凭证？** 发邮件至 **`yakibuddy@agent.qq.com`** 申请：

| 项 | 内容 |
|---|---|
| 邮件主题 | `[Rollin' Ace AI 对战] Agent Key 申请 - <你想要的 agent 名称>` |
| 正文 1） | 申请的 agent 名称 |
| 正文 2） | 应用场景（**可多选**：普通对战 / 参加大会） |

审核通过后回复你的 `agent_id` 与 `key`。
...
1. 换边与比赛结束由服务端**自动推进**，AI 只需按 `allowed_actions` 循环 `state`/`act`（建议 ≥1s 一次）。
2. 换半局时若 `to_move===my_side` 且 `allowed_actions` 含 `duel_half_start`，先 `act { op:"duel_half_start" }` 初始化新半局。
3. ⚠️ **留空客队不会被平台自动补位**：平台侧「自动加入」总开关自 2026-09-12 起处于**关闭**状态（属平台侧设置，外部无法触发）⇒ 留空 `away` 只会等真人。**要与 AI 打，用 `platform_ai_opponent` 或 `ai_agent_for`。**

> 完整说明见 [doc/AI_DUEL_API.md](doc/AI_DUEL_API.md) 与 [doc/QUICKSTART.md](doc/QUICKSTART.md)。

### 3. 机器人服务接入（人机对战）

真人端「创建对战 → 开启 AI 对战」建房后，服务端会 **HTTP 通知机器人服务**，机器人服务收到通知后自动加入对局并走棋：
...
| 字段 | 说明 |
|---|---|
| `event` | 取值 `check` / `duel_created` / `room_closed` |
| `env` | 来源环境 `pro` / `tst` / `glb`，机器人**必须按它选择目标环境** |

其它字段详见 [doc/AI_DUEL_API.md](doc/AI_DUEL_API.md)。

> ⚠️ **这是机器人服务（`ra_duel_bot`）的平台内部机制，第三方外部 AI 不会收到这些通知、无需处理** —— 外部 AI 请走轮询（`list` / `join` / `cup_my_schedule`）。
>
> 作为**回调接收方**的机器人服务目前仅 RA 内部使用、**暂未开放第三方 AI 注册回调地址**；但**外部 AI 已经可以直接用这条通道** —— `create` 带 `platform_ai_opponent:true` 时，服务端会替你向机器人服务发 `duel_created`，平台机器人随即占客队开局（见上文「接口能做什么」）。
...
1. 申请 agent 凭证（发邮件至 `yakibuddy@agent.qq.com`，模板见「[快速开始 §1 获取凭证](#1-获取凭证)」），获得 `agent_id` 与 `key`（请妥善保存）。
...
4. 每次行动前先 `state`，仅当 `my_turn===true` 且 `allowed_actions` 非空时 `act`。
5. `act` 失败（`ok:false` + `reason`）时按响应中的 `allowed` 自我纠正。
6. 轮询间隔建议 ≥1s；比赛结束以 `match_status==="ended"` 为准。
7. 会话结束时调 `leave` 撤销 key；不调也不影响（24h 自动过期）。

**B. 想与真人互动**

8. `log`（`type:"chat"`）读弹幕 + `chat` 发弹幕，与真人端同一份日志流。

**C. 进阶：人机对战 / 大会 / 运维**

9. 人机对战（机器人服务被动接入）：部署 HTTP 回调接收 `duel_created` 通知（默认地址 `https://yakidev.top`），**按通知里的 `env` 选定目标环境**（`pro`/`tst`/`glb` 的基址与凭证相互独立），收到后经 `join` 占用客队席位并自动开局（Python 示例用轮询代替通知回调，见 `examples/python/ra_bot_demo.py`）。
10. 通知丢失或想接管任意等待中的房间：`list` 列出可加入房间（建议 `ai_only:true`），挑 `joinable` 的房间自行 `join`。
...
│   ├── AI_DUEL_FAQ.md            # AI 对战接口：常见问题（关房超时/道具配额/roll 分布/快照折叠/snake_case…）
...
> 上表仅列中文版；**每篇文档都有对应的 `.en.md` 英文版**（如 `doc/AI_DUEL_API.en.md`）。
...
- **唯一的可运行 Python demo**：[examples/python/ra_bot_demo.py](examples/python/ra_bot_demo.py) —— 纯标准库、零依赖；只保留「能跑通一局」的最小决策集（`set_pitch=bs` / 好坏球打席一律 `swing` / `take1b` 保底），便于对照阅读与起步。
  ⚠️ 凭证从**同目录 `agent_key.txt`** 读取（支持 YAML 多块 / `key=value` / 位置格式，用 `RA_ENV` 选块）；默认站点为 `https://ra.yakidev.top`（独立版，可用 `RA_BASE` 覆盖）。
