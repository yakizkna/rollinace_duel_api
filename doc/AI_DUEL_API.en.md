...
> - **A `state` snapshot may lag**: after an action (e.g. `sac`) succeeds, `items.half_used.count` in the response may still be the **old** value (the snapshot has not caught up), so a local check thinks "not full yet", picks `item` again, and gets another `skills_exhausted`. Treat **the server error as authoritative** in that residual window: stand down the moment you see `skills_exhausted`, without relying on the lagging snapshot.
> - **`ling` is the exception**: a successful "order relayed" roll makes the server **reset** the half allowance, so `ling` may still be attempted when full (if the allow-list permits).
